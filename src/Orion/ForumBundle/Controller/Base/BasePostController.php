...
{
    /*
     * @var object $post Orion\ForumBundle\Entity\Post
     */
    protected $post;
    ...
    protected function redirectAfterPost(SlidingPagination $posts) {
        ...
        $paramPage  = $this->container->getParameter('orion_forum.pagination')["page_name"];

        $url = $this->generateUrl('orion_forum_post', array(
    ...
    protected function autorizedPostForm(SlidingPagination $posts, Request $request, Form $form) {
        if ($posts->getPageCount() == $request->query->get( $this->container->getParameter('orion_forum.pagination.pagename') , 1)) {
     ...
     * @param objet $request Symfony\Component\HttpFoundation\Request
     * @param object $form Symfony\Component\Form\Form
     * @param objet $post Orion\ForumBundle\Entity\Post Listing posts in pagination
     * 
     * @return false|object Orion\ForumBundle\Entity\Post
     */
    protected function getPreview(Request $request, Form $form, Post $post) {
       if ( false === $this->container->getParameter('orion_forum.preview') ) {
            return false;
        }
        if ( $form->get('preview')->isClicked() ) {
            $request->getSession()->getFlashBag()->add('warning', $this->getTranslator()->trans('orion.forum.warning.preview'));
            return $post; 
        }
    ...
    protected function addQuote($quote) {
        $post = $this->getEm()->getRepository('OrionForumBundle:Post')->find($quote);
        if ($post === NULL) {
            return NULL;
        }
