<?php
namespace Orion\ForumBundle\Controller;

use Orion\ForumBundle\Controller\Base\BaseController;
use Sensio\Bundle\FrameworkExtraBundle\Configuration\Route;
use Sensio\Bundle\FrameworkExtraBundle\Configuration\Security;


/**
 * @author David Verdier <contact@orion.com>
 * 
 * Class AdminController
 * 
 * METHODS LIST:
 * indexAction() return orion_forum_admin_dashboard
 * newCategoryAction(Request $request) Create a new category
 * editCategoryAction(Request $request, $id) Edit a category
 * removeCategoryAction(Request $request, $id) Drop a category
 * newForumAction(Request $request, $cid) Create a new forum
 * editForumAction(Request $request, $id) Edit a forum
 * removeForumAction(Request $request, $id) Drop a forum
 * getRolesList() Return list of symfony roles
 * 
 */
class AdminController extends BaseController
{

    /**
     * 
     * Moderator's dashboard
     * 
     * @Route("/admin", name="orion_forum_admin_dashboard")
     * @Security("is_granted('ROLE_MODERATOR')")
     * 
     */
    public function indexAction()
    {
        $em = $this->getEm();
        $posts = $em->getRepository('OrionForumBundle:Post')->findBy(array(), array('date' => 'desc'));
        $topics = $em->getRepository('OrionForumBundle:Topic')->findBy(array(), array('date' => 'desc'));
        if ($this->getAuthorization()->isGranted('ROLE_ADMIN')) {
            $forums = $em->getRepository('OrionForumBundle:Forum')->findAll();
            $categories = $em->getRepository('OrionForumBundle:Category')->findBy(array(), array('position' => 'desc', ));
        } else {
            $forums = NULL;
            $categories = NULL;
        }

        return $this->render('OrionForumBundle::Moderator/index.moderator.html.twig', array(
            'posts' => $posts,
            'topics' => $topics,
            'forums' => $forums,
            'categories' => $categories
        ));
    }
}
